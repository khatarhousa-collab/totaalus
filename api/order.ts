    ...
    if (data.status === 'Order placed' && resultPageId && !alreadyPlaced) {
      ...
      if (isNL && data.email && process.env.RESEND_API_KEY) {
        ...
        try {
          ...
          try {
            const value = await get('totaalus_whatsappNumber');
            if (typeof value === 'string' && /^\d{8,15}$/.test(value)) {
              whatsappNumber = value;
            }
          } catch {
            // Edge Config not configured / unreachable — use the fallback silently.
          }
